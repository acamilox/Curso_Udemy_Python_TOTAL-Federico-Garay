...
# Empezamos pidiendo el texto y las letras
texto = input("Por favor, ingresa un texto cualquiera: ")
letra1 = input("Ingresa la primera letra a buscar: ")
letra2 = input("Ingresa la segunda letra a buscar: ")
letra3 = input("Ingresa la tercera letra a buscar: ")
...
diccionario_python = {True: "sí", False: "no"}
contar_letras = {}

# 1. Contar cuántas veces aparece cada letra
for letra in letras_minusculas:
    contar_letras[letra] = texto_minusculas.count(letra)
print(f"\nAnálisis del texto:\n")

for letra, cuenta in contar_letras.items():
    print(f"La letra '{letra}' aparece {cuenta} veces.")

# 2. Contar cuántas palabras hay en el texto
num_palabras = len(palabras)
print(f"\nEl texto contiene {num_palabras} palabras.")

# 3. Primera y última letra del texto
primera_letra = texto[0] if texto else "El texto está vacío"
ultima_letra = texto[-1] if texto else "El texto está vacío"
print(f"\nLa primera letra del texto es: '{primera_letra}'")
print(f"La última letra del texto es: '{ultima_letra}'")

# 4. Texto con el orden de las palabras invertido
texto_invertido = ' '.join(palabras_invertidas)
print(f"\nEl texto con las palabras en orden invertido es:\n{texto_invertido}")

# 5. Verificar si la palabra "Python" está en el texto
contiene_python = "python" in texto_minusculas
print(f"\n¿El texto contiene la palabra 'Python'? {diccionario_python[contiene_python]}")
